...
async function getMyProfile(req, res) {
	try {
		...
		if (!profile) return res.json({ user: await User.findById(req.user.id).select('name email'), bio: '', skills: [], linkedIn: '', walletAddress: '' });
		return res.json(profile);
	} catch (error) {
		return res.status(500).json({ message: 'Server error', error: error.message });
	}
...
async function upsertMyProfile(req, res) {
	try {
		const { bio, linkedIn, skills, walletAddress, location } = req.body;
